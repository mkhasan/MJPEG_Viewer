...
int test_main(int s)
{
    ...
    if (getsockname(s, (struct sockaddr*)&si_me, (socklen_t *)&slen) < 0) 
        return -1;
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    printf("fReadSoure is null is it ? %d \n", fReadSource == NULL);
    ...
    int ret = test_main(fRTPSocket->socketNum());
