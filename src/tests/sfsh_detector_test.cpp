...
int main(int argc, char *argv[]){
	...
	fileReader = make_unique<FileReader>("/home/hasan/raw_data/smoke_detector/jpegs", 100, 61);

	std::vector<detection_results_t> results;


	string model_path = string(getenv("TF_ROOT")) + "/tensorflow/k2tf_projects/sfsh_detector/models";

	auto yolov3 = model_path + "/" + "yolov3-voc_final.pb";
	...
	detector = std::make_unique<SFSH_Detector>(argv[0], JPEG_ENCODING, yolov3_model);
