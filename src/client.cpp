...
std::unique_ptr<AbsDetector> net;
...
const int MAX_BUFF_SIZE = ImgData::WIDTH*ImgData::HEIGHT*3+1024;
static char imageData[MAX_BUFF_SIZE];
...
public:
	...
	sem_t mutex_lock;
	Data();
	void SetData(const char *data, int len, const ImgData::image_header& header);
	void GetData(char *data, int& len, ImgData::image_header& header);
};
...
void Data::SetData(const char *data, int len, const ImgData::image_header& header_) {

	assert(len < sizeof(imageData));
	sem_wait(&mutex_lock);
...
void Data::GetData(char *data, int& len, ImgData::image_header& header_) {
	sem_wait(&mutex_lock);
...
// "rtsp://192.168.100.87:8554/mjpeg/1"
int main(int argc, char** argv) {


	if (argc < 2) {
		cout << "stream missing" << endl;
		return -1;
	}
  // Begin by setting up our usage environment:
	...
	try {
		...
		auto serverIP = res[1].substr(2);
		//cout << "sevcer ip is " << serverIP << endl;
		//cout << "len is " << serverIP.length() << endl;
		...
		//serviceContainer->set<DETECTION_SENDER>(serverIP, DETECTION_READER_CUSTOM_PORT);
        serviceContainer->set<DETECTION_SENDER>();

    bool useDetectors[DetectionInfo::MAX_CLASS] = {false, };
    for (int i=0; i< DetectionInfo::MAX_CLASS; ++i)
      useDetectors[i] = true;

		//net = std::make_unique<FakeDetector>(useDetectors, serviceContainer->get<DETECTION_SENDER>());
        net = std::make_unique<FakeDetector>(serviceContainer->get<DETECTION_SENDER>());



	  // We need at least one "rtsp://" URL argument:
		if (argc < 2) {
			THROW_EX(RobotException, "Error in argument parsing");
			return 1;
		}
...
DummySink::DummySink(UsageEnvironment& env, MediaSubsession& subsession, char const* streamId)
  : MediaSink(env),
    fSubsession(subsession), index(0) {
...
static void* detector_thread(void* arg)
{
	static char jpegData[MAX_BUFF_SIZE];
	ImgData::image_header header;
	int dataLen;
	int lastSeq = -1;
	...
	while(quit == false) {
		commonData.GetData(jpegData, dataLen, header);

        
        if (count < 50 && dataLen > 0 && 0) {
            printf("data len is: %d \n", dataLen);
            char filename[256];
            sprintf(filename, "./temp/test%06d.jpg", count);
            cout << filename << endl;
            ofstream wf(filename, ios::out | ios::binary);
            if (!wf) {
                cout << "Cannot open file: " << filename << endl;
                ...
                wf.write((char *) jpegData, dataLen);
        ...
        if (dataLen > 0) {
            ...
            Mat rawData( 1, dataLen, CV_8UC1, (void*)jpegData);
            ...
            if ( decodedImage.data != NULL )   
            {
                ...
                imshow(window_name, decodedImage);
                int ret = waitKey(100);
                if (ret == 27)
                    break;
